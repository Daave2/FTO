...
def screenshot_on(page: Page, name: str):
    """Save a full-page screenshot with a timestamped filename."""
    path = OUTPUT_DIR / f"{name}_{timestamp()}.png"
    try:
        asyncio.get_event_loop().create_task(page.screenshot(path=str(path), full_page=True))
        logger.info(f"Saved screenshot: {path}")
    except Exception as e:
        logger.error(f"Failed to save screenshot '{path}': {e}")
...
def create_google_chat_card(title: str, subtitle: str, items: list, link_text: str, link_url: str) -> Dict:
    if items and items[0].strip().startswith("<pre>"):
        ...
        text_content = "<br>".join(str(i) for i in items if i is not None)
    return {
        "cardsV2": [{
...
def send_google_chat_message(title: str, subtitle: str, summary_items: list, link_text: str, link_url: str):
    if not GOOGLE_CHAT_WEBHOOK:
        logger.warning("Google Chat webhook not configured; skipping message.")
        return
    ...
    try:
        ...
        if resp.status_code not in (200, 204):
            ...
            logger.info(f"Sent Google Chat: {title}")
    except Exception as e:
...
async def perform_login(page: Page) -> bool:
    try:
        ...
        screenshot_on(page, "login_timeout")
        return False

    except Exception as e:
        logger.error(f"Login error: {e}", exc_info=True)
        screenshot_on(page, "login_error")
        return False
...
async def login_and_get_context() -> Optional[BrowserContext]:
    ...
    try:
        ...
        logger.error(f"login_and_get_context error: {e}", exc_info=True)
...
async def extract_osp_data(context: BrowserContext) -> Optional[Dict[str, Dict[str,str]]]:
    ...
    try:
        ...
        for td in target_dates:
            ...
            while True:
                ...
                try:
                    ...
                    logger.info(f"No orders found on page {page_num} for {date_str}")
                    break
                ...
                for ref in new_refs:
                    ...
                    try:
                        ...
                        logger.error(f"Error opening details for {ref}: {e}")
                        screenshot_on(page, f"open_error_{ref}")
                        seen.add(ref)
                        await page.goto(url, timeout=20000, wait_until='domcontentloaded')
                        continue

                    # Screenshot detail page
                    screenshot_on(page, f"details_{ref}")
                    ...
                    for sel in ['main','div[role="main"]','article','section#main-content','div.page-content','div.order-detail-container','body']:
                        try:
                            ...
                            if await loc.count() > 0:
                                ...
                                if ("Order contents" in txt and "Product ID" in txt) or ("Collection slot:" in txt):
                                    full_text = txt
                                    break
                        except Exception:
                            continue
        ...
        if orders_data:
            ...
            logger.info(f"Saved extracted_orders_data.csv ({len(orders_data)} orders).")
            if FORM_URL:
        ...
        if page and not page.is_closed():
            screenshot_on(page, "extract_error")
        return None
...
async def generate_daily_item_summary(orders_data: Optional[Dict[str,Dict[str,str]]], prod_df: Optional[pd.DataFrame]) -> Optional[str]:
    ...
    total = 0
    texts = []
    for info in (orders_data or {}).values():
...
async def main() -> bool:
    ...
    if not ctx:
        logger.error("Could not obtain browser context; aborting.")
        return False
    ...
    if orders is None:
        logger.error("Extraction failed completely; aborting.")
        return False
    ...
    for i in range(1,4):
        ...
        if cnt: any_orders = True
        counts.append(f"{cnt} orders for {d.strftime('%d/%m/%y')}")
    if not any_orders:
