...
def setup_logging():
    ...
    fh = logging.FileHandler(OUTPUT_DIR / 'extractor.log'); fh.setFormatter(fmt); logger.addHandler(fh)
    return logger
...
async def dump_page_state(page: Page, name: str):
    ...
    try:
        await page.screenshot(path=str(png), full_page=True); logger.info(f"🖼 Screenshot saved: {png}")
    except Exception as e:
        logger.error(f"❌ Screenshot failed ({png}): {e}")
    try:
        content = await page.content()
        async with aiofiles.open(html, 'w') as f: await f.write(content)
    ...
    return {
        "cardsV2": [{
            ...
            "card": {
                ...
                "sections": [{"header": "Upcoming Order Totals", "widgets": widgets},
...
def create_item_summary_card(
    ...
    if by_dept and prod_df is not None and not prod_df.empty:
        ...
        sections.append({"header": "Item Details", "widgets": [{"textParagraph": {"text": summary_data.get("summary_text", "Summary not available.")}}]})
...
def send_orders_email(file_path: Path, summary_data: Optional[Dict[str, Any]] = None,
    ...
    try:
        ...
        if port == 465:
            ...
            server = smtplib.SMTP(SMTP_SERVER, port); server.starttls()
...
async def perform_login(page: Page) -> bool:
    try:
        ...
        await dump_page_state(page, "login_timeout"); return False
    except Exception as e:
        logger.error(f"❗Login error: {e}", exc_info=True)
        await dump_page_state(page, "login_error"); return False

async def login_and_get_context() -> Optional[BrowserContext]:
    global browser_instance
    if not browser_instance:
        logger.critical("Browser not initialized."); return None
    ...
    if "login.sso.osp.tech" in page.url:
        ...
        await page.close(); await ctx.close(); return None
    else:
...
async def extract_osp_data(context: BrowserContext) -> Optional[Dict[str,Dict[str,str]]]:
    ...
    try:
        ...
        for d in dates:
            ...
            while True:
                ...
                if nxt and await nxt.is_enabled():
                    ...
                    break
        if data:
            df = pd.DataFrame.from_dict(data,orient='index'); df.index.name = 'Order Reference'
            df.to_csv(OUTPUT_DIR / 'extracted_orders_data.csv'); logger.info(f"Saved CSV ({len(data)} orders).")
            if FORM_URL:
        ...
        if page and not page.is_closed(): await dump_page_state(page, "extract_error")
        return None
    finally:
...
async def generate_daily_item_summary(orders, prod_df) -> Optional[Dict[str, Any]]:
    ...
    total_orders = 0; texts = []
    for info in (orders or {}).values():
        if info.get('collection_slot') == key:
            total_orders += 1; texts.append(info['details'])
    matched_orders = 0
    for t in texts:
        found = False
...
async def main() -> bool:
    ...
    await ctx.close()
    if orders is None:
        logger.error("Extraction failed, orders data is None."); return False
    ...
    for i in range(1, 4):
        ...
        if count > 0: any_orders = True
        counts.append(f"<b>{count} orders</b> for {d.strftime('%a, %d %b')}")
    if not any_orders: counts = ["No orders found for the next 3 days."]
    ...
    if summary_data:
        ...
        summary_payload = create_item_summary_card(f"OSP Item Summary for {tomorrow_str}", f"Ran at: {ts} (UK)", summary_data, dashboard_url, prod_df=product_lookup_df)
        send_google_chat_message(summary_payload, f"Item Summary for {tomorrow_str}")

    # Email
    send_orders_email(OUTPUT_DIR / 'extracted_orders_data.csv', summary_data=summary_data, prod_df=product_lookup_df)

    await browser_instance.close(); await playwright_instance.stop()
    return True
